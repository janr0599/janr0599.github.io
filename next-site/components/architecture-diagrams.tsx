...
function useDashOffset(value: MotionValue<number>) {
	...
	useEffect(() => {
		if (ref.current)
			ref.current.style.strokeDashoffset = String(value.get());
	}, [value]);
	...
	return (
		<>
			...
			>
				...
				>
					<header className="flex min-h-[3.75rem] flex-col justify-center gap-1 pt-5 sm:flex-row sm:items-baseline sm:justify-between sm:gap-6 lg:pt-0">
						<h3 className="text-[17px] font-medium tracking-[-0.01em] sm:text-lg">
							{project.title}
						</h3>
						<p className="shrink-0 text-[13px] text-accent">
							{project.result}
						</p>
					</header>
					...
					{last ? (
						<div className="border-t border-rule py-6">
							{links.portfolioPdf ? (
								...
								>
									Download the PDF portfolio, with more
									projects and detail
									<ArrowUpRight
										size={16}
										weight="bold"
								...
								<p className="inline-flex items-center gap-2 text-[15px] text-paper-3">
									PDF portfolio with more projects and detail,
									coming soon
									<ArrowUpRight
										size={16}
										weight="bold"
			...
			<div className="mx-auto max-w-[1400px] px-4 pt-28 pb-12 sm:px-8 lg:pt-0 lg:pb-16">
				...
				>
					<div ref={head} className="stack-head lg:pt-24 lg:pb-12">
						<Reveal>
							...
							>
								Four production systems, drawn as they run.
								Scroll to send a request through each one. Hover
								or focus a node to see what it does.
							</Item>
						</Reveal>
					</div>
				</div>
				...
				<Tooltip.Provider delayDuration={200} skipDelayDuration={600}>
					<div
						...
						style={
							{ "--pad": `${stackPx}px` } as React.CSSProperties
						}
					>
						{projects.map((p, i) => (
							<ProjectCard
